...
not 1.96), and always reports how much more data would settle the question.
...
well-calibrated model looks like this: gaps scattered around zero. Every
bucket sitting *below* the line is the signature of systematic overconfidence
...
It also warns you not to act on it. Cutting the worst bucket *after* seeing
which bucket was worst is in-sample fitting, and it is how backtests get
retracted.

### 5. Power — how much data would settle it?

Tells you how many resolved trades are needed to detect an edge the size of the
one being claimed, at 95% confidence and 80% power — so you can size a paper
run before it starts instead of arguing about it afterwards.
...
| `--price` | entry price (breakeven, for probability-priced instruments) | unit economics |
...
```bash
# a real edge, roughly half of it consumed by fees -- survivable
...
cd edgecheck && pip install -e '.[dev]' && pytest
```
